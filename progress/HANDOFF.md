...
- Ed25519 strict, blake3, RFC 8949 deterministic CBOR subset, no unknown fields.
  Addresses: version byte, 32 byte payload, 4 byte checksum, base32.
- Manifests and the pointer named `manifest` are root signed only, device
  ...
  head order is highest `seq`, then `created`, then lowest address.
...
- Tauri ignores child web view positions on Linux; see `frame` and `pack`.
  WebKit routes only `weft://blob/<address>` to the scheme handler.
