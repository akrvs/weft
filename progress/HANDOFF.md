...
- Gateway: plain HTTP on an explicit bind, default loopback, GET and HEAD only except POST
  ...
  requests use `Resolver::offline` and answer 404 `...; log in to fetch from relays`. A session
  pulls under a `Semaphore` of `--pulls` (4), past it 429, and a memory only `Budget` of
  `--budget` MiB (64, 0 unlimited) per root per fixed hour, counted from relay bytes through
  `Resolver::metered`, charged after the route; over budget it reads offline and a miss says
  `; pull budget spent, resets at <time>`. Sessions: `<home>/gateway/sessions`, canonical CBOR,
  mode 0600, rewritten on every login and logout, at most 1024. `--allow <path>`: one root address
  per line, read at start and on SIGHUP, which also sweeps expired sessions to disk.
...
- The store is a daemon on `<home>/store.sock` mode 0600, relay framing, nonce handshake under
  ...
  connections. That key skips grants and alone may use the browser only requests in `docs/store.md`.
  Neither client signs or opens `records/` or `blobs/`; the browser pushes to relays itself.
  `serve --attach` reads the passphrase from stdin, exits on EOF; the browser shows a 16 KiB ring.
